...
# We're naming function fetch_users
def fetch_users():
# We'll start with an empty user list
  ...
  # If they have a UID of over 1000, and home is listed in the password directory
    ...
    # If those are true, then we're going to run a user.append and create a dictionary
    #entry with four attributes: name, id, home, and shell.
      users.append({
	  'name': user.pw_uid,
          'id': user.pw_uid,
          'home': user.pw_dir,
